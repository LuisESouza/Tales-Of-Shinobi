...
public class FireJetJutsu implements Jutsu {
    public static final FireJetJutsu INSTANCE = new FireJetJutsu();
    private static final ScheduledExecutorService SCHEDULER = Executors.newScheduledThreadPool(2);

    private static final String WATER_JET_PARTICLE_ID = "Flame_Jet";
    ...
    private FireJetJutsu() {}

    @Override public String getId() { return "katin_fire_jet"; }
    @Override public String getDisplayName() { return "Katon: Suiryū Shōten no Jutsu"; }
                ...
                world.execute(() -> {
                    try {
                        ...
                        if (initialPos.distance(currentPos) > MOVE_THRESHOLD) {
                            stopWaterJet(isChanneling, taskHolder[0]);
                            playerRef.sendMessage(Message.raw("Katon canceled: You moved!").color(Color.YELLOW));
                            return;
                        }
                        ...
                        if (playerData == null || playerData.getCurrentChakra() < CHAKRA_COST_PER_TICK) {
                            stopWaterJet(isChanneling, taskHolder[0]);
                            playerRef.sendMessage(Message.raw("Katon canceled: Insufficient chakra!").color(Color.RED));
                            return;
                        }
                        ...
                        java.util.List<Ref<EntityStore>> nearbyPlayers = TargetUtils.getPlayersInRadius(spawnPos, 50.0, store);
